...
**cenetex/agent** - GitHub App that automatically handles issues and PRs labeled with `agent`. Runs Codex in AWS Fargate to implement features, fix bugs, and review code.

## Architecture

- **Webhook Handler** (`infra/lib/webhook-handler.ts`) - Lambda that listens for GitHub webhook events (issues/PRs labeled `agent`)
- **Agent Container** (`agent/`) - Docker container running Codex, spawned in Fargate
...
- `agent/entrypoint.sh` - Wrapper script for Codex
